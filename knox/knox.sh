...
function add_presto(){
  mkdir -p ${GATEWAY_HOME}/data/services/presto/0.0.1/
  cat << 'EOF' > ${GATEWAY_HOME}/data/services/presto/0.0.1/rewrite.xml
<rules>
    <rule dir="IN" name="PRESTO/presto/inbound/root" pattern="*://*:*/**/presto/{**}?{**}">
        <rewrite template="{$serviceUrl[PRESTO]}/{**}?{**}"/>
    </rule>
</rules>
EOF
  cat << 'EOF' > ${GATEWAY_HOME}/data/services/presto/0.0.1/service.xml
<service role="PRESTO" name="presto" version="0.0.1">
    <routes>
        <route path="/presto/**?**">
        </route>
    </routes>
